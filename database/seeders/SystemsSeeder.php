...
class SystemsSeeder extends Seeder
{
    private const DATA_FILES = [
        'database/seeders/data/malik_system_seed_data.json',
        'database/seeders/data/vetora_system_seed_data.json',
    ];
    ...
    private function systemDefinitions(): array
    {
        return array_map(function (string $file): array {
            try {
                ...
                throw new \RuntimeException("The approved system seed data file [{$file}] is invalid JSON.", previous: $exception);
            }

            if (! is_array($data['system'] ?? null)) {
                throw new \RuntimeException("The approved system seed data file [{$file}] has no system payload.");
            }
