...
class MalikCaseStudySeeder extends Seeder
{
    private const DATA_FILE = 'database/seeders/data/malik_case_study_seed_data.json';
    ...
    public function run(): void
    {
        ...
        if (! $system) {
            throw new \RuntimeException("The approved Malik system [{$relations['system_slug']}] must exist before seeding the case study.");
        }

        $service = Service::query()->where('slug', $relations['service_slug'])->first();
        if (! $service) {
            throw new \RuntimeException("The approved service [{$relations['service_slug']}] must exist before seeding the Malik case study.");
        }

        $system->forceFill([
            'tech_stack' => $payload['related_system_patch']['tech_stack'],
        ])->save();
    ...
    /**
     * @return array{
     *     related_system_patch: array{slug: string, tech_stack: list<string>},
     *     case_study: array<string, mixed>
     * }
     */
    private function payload(): array
    {
        try {
            ...
            throw new \RuntimeException('The approved Malik case study seed data is invalid JSON.', previous: $exception);
        }

        if (! is_array($payload['case_study'] ?? null)) {
            throw new \RuntimeException('The approved Malik case study seed data has no case study payload.');
        }
